        ...
        {phase === "memorize" ? (
          ...
          <>
            ...
            <div className="flex flex-wrap justify-center gap-4" role="group" aria-label="Items to check">
              {items.map((item, i) => {
                let style = "bg-surface border-2 border-border hover:border-primary hover:bg-primary/5";
                if (selected !== null) {
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer rounded-xl p-4 text-5xl transition-all focus:outline-none focus:ring-4 focus:ring-primary/30 ${style}`}
                    aria-label={`Item ${i + 1}`}
                  >
                    {item}
                  </button>
            ...
            {selected !== null && (
              ...
              >
                {selected === round.changedIndex
                  ? "Correct! Good eye!"
                  : `The changed item was at position ${round.changedIndex + 1}`}
              </div>
            )}
          </>
        )}
